...
type AddAntinodesFn =
    fn((isize, isize), (isize, isize), (isize, isize), &mut HashSet<(isize, isize)>);
...
fn add_antinodes(
    ...
    if in_bounds(size, (a.0 - diff.0, a.1 - diff.1)) {
        antinodes.insert((a.0 - diff.0, a.1 - diff.1));
    }

    if in_bounds(size, (b.0 + diff.0, b.1 + diff.1)) {
        antinodes.insert((b.0 + diff.0, b.1 + diff.1));
    }
}
...
fn count_antinodes(input: &str, add_antinodes: AddAntinodesFn) -> usize {
    ...
    input.lines().enumerate().for_each(|(j, row)| {
        row.chars()
            .enumerate()
