...
fn add(lhs: (usize, usize), rhs: (i32, i32)) -> (usize, usize) {
    ((lhs.0 as i32 + rhs.0) as usize, (lhs.1 as i32 + rhs.1) as usize)
}
...
fn get_areas_perimiters(input: &str) -> (Vec<usize>, Perimiters) {
    ...
    for i in 1..width-1 {
        for j in 1..height-1 {
            if unique.contains(&(i,j))  { continue; }
            ...
            while !queue.is_empty() {
                ...
                if unique.contains(&current) { continue; }
                ...
                *areas.last_mut().unwrap() += 1;
                ...
                for dir in [(1, 0), (-1, 0), (0, 1), (0,-1)] {
                    ...
                    if grid[next.1][next.0] == grid[j][i] {
                        ...
                        perimiters.last_mut().unwrap().entry(dir).or_insert_with(Vec::new).push(next);
                    }
                }
            }
        }
